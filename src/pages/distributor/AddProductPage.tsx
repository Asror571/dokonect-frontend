...
const AddProductPage = () => {
  ...
  const {
    ...
    defaultValues: { status: 'ACTIVE', unit: 'dona' },
  });
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (valid.length !== files.length)
      toast.error("Ba'zi rasmlar 5MB dan katta");
    if (images.length + valid.length + existingImages.length > 10) {
      toast.error('Maksimal 10 ta rasm');
      return;
    }
  ...
  // Rasm serverga yuklash (agar backend qo'llab-quvvatlasa)
  const uploadImages = async (): Promise<string[]> => {
    ...
    try {
      ...
      // Upload endpoint yo'q bo'lsa, rasmlarni base64 sifatida qaytaradi
      return [];
    }
  };
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Mahsulot parametrlarini to'ldiring
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Input
                label="Mahsulot nomi *"
                placeholder="Masalan: Galaxy S24"
                error={errors.name?.message}
                {...register('name')}
              />
              <Input
                label="SKU / Kod"
                placeholder="Bo'sh qoldiring — avtomatik"
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {existingImages.map((img: any, i: number) => (
                <div
                  key={`exist-${i}`}
                  className="relative w-28 h-28 border border-slate-200 rounded-xl overflow-hidden group"
                >
                  <img
                    src={img.url || img}
                    alt="product"

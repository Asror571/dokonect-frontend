...
import { TrendingUp, ShoppingBag, Truck, Download } from 'lucide-react';
  ...
  return (
    <div className="fade-in space-y-8 max-w-7xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 underline decoration-indigo-500 decoration-2 underline-offset-8 uppercase tracking-widest">Savdo va Tahlil</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Umumiy Savdo',   value: `${(stats.todaySales?.amount||stats.totalRevenue||0).toLocaleString('uz-UZ')} UZS`, icon: TrendingUp, bg: 'bg-indigo-50',  ring: 'bg-indigo-50',  trend: '+12.5% o\'sish',   tc: 'text-emerald-500' },
          { label: 'Buyurtmalar',    value: `${stats.orders?.total||0} ta`,   icon: ShoppingBag, bg: 'bg-amber-50',   ring: 'bg-amber-50',   trend: 'Faollik yuqori',  tc: 'text-amber-500'   },
          { label: 'Haydovchilar',   value: `${stats.activeDrivers||0} faol`, icon: Truck,       bg: 'bg-sky-50',     ring: 'bg-sky-50',     trend: 'Harakatdagi',     tc: 'text-sky-500'     },
          { label: "O'rtacha chek",  value: `${(stats.avgOrderValue||125_000).toLocaleString('uz-UZ')} UZS`, icon: TrendingUp, bg: 'bg-emerald-50', ring: 'bg-emerald-50', trend: 'Barqaror', tc: 'text-emerald-500' },
        ].map((card) => (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topProducts} layout="vertical" margin={{ left: 50 }}>
                ...
                <Bar dataKey="value" fill="#6366f1" radius={[0, 8, 8, 0]} barSize={25}>
                  {topProducts.map((_: any, i: number) => <Cell key={`cell-${i}`} fill={COLORS[i % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>

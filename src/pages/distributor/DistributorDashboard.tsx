    ...
    <div className="fade-in space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => navigate('/distributor/products/add')} className="flex items-center gap-2 px-4 py-2 bg-sky-500 text-white rounded-xl hover:bg-sky-600 transition-all font-medium text-sm shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Bugungi tushum',   value: `${(stats.todaySales?.amount || stats.todaySales || 0).toLocaleString('uz-UZ')} UZS`, icon: DollarSign,   bg: 'bg-emerald-50', c: 'text-emerald-500' },
          { label: 'Faol buyurtmalar', value: `${stats.orders?.inProgress || stats.orderStats?.new || 0} ta`,                        icon: Package,       bg: 'bg-sky-50',     c: 'text-sky-500'     },
          { label: 'Kam qolganlar',    value: `${stats.lowStockAlerts || stats.lowStockCount || 0} tur`,                              icon: AlertTriangle, bg: 'bg-amber-50',   c: 'text-amber-500', onClick: () => navigate('/distributor/inventory') },
          { label: "O'sish",           value: `${growth > 0 ? '+' : ''}${growth.toFixed(1)}%`,                                       icon: TrendingUp,    bg: growth >= 0 ? 'bg-green-50' : 'bg-red-50', c: growth >= 0 ? 'text-green-500' : 'text-red-500' },
        ].map((card) => (
          <div key={card.label} onClick={(card as any).onClick} className={`bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow ${(card as any).onClick ? 'cursor-pointer' : ''}`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col">
          ...
          <div className="space-y-3 flex-1">
            {recentOrders.length > 0 ? recentOrders.map((order: any) => (
              <div key={order.id} onClick={() => navigate(`/distributor/orders/${order.id}`)} className="flex flex-col p-3.5 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-colors cursor-pointer">
            ...
            )) : <div className="flex items-center justify-center flex-1 text-slate-400 text-sm h-32">Buyurtmalar yo'q</div>}
          </div>
        </div>
      </div>
    </div>

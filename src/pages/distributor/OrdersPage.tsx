...
import { MapPin, PackageCheck, Loader2, Eye, Search, CheckCircle, XCircle, Truck, X, RefreshCw } from 'lucide-react';
...
const DistributorOrdersPage = () => {
  ...
  // ── Fetch orders — 15 sekundda bir yangilanadi ────────────────────────────
  const { data: fetchRes, isLoading, isFetching, dataUpdatedAt } = useQuery({
    ...
    refetchIntervalInBackground: true,      // ← background da ham ishlaydi
    retry: false,
  });
    ...
    <div className="fade-in">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-400">
              ...
              <span>{isFetching ? 'Yangilanmoqda...' : `Oxirgi yangilanish: ${lastUpdated}`}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['distributor-orders'] })}
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  {['Buyurtmachi', 'Sana', 'Manzil', 'Summa', 'Holat', 'Boshqaruv', ''].map((h) => (
                    <th key={h} className="px-5 py-3.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
      ...
      {acceptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-3xl shadow-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setAcceptModal(null)} className="flex-1 py-2.5 border border-slate-200 rounded-2xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">
                Bekor
              </button>

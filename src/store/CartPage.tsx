...
import {
  Trash2, Plus, Minus, ShoppingCart, MapPin,
  ArrowRight, Package,
} from 'lucide-react';
...
const CartPage = () => {
  ...
  const handleCheckout = () => {
    ...
    mutate({
      ...
      deliveryAddress: { address },        // ← API object kutadi
      notes:           note || undefined,
      paymentMethod,
      items: items.map((i) => ({
        productId:  i.productId,
        variantId:  i.variantId,           // ← agar variant bo'lsa
        quantity:   i.quantity,
      })),
    });
  };
    ...
    return (
      <div className="page flex flex-col items-center justify-center min-h-[60vh] text-center fade-in">
        ...
        <p className="text-slate-500 text-sm mb-6 max-w-xs">
          Katalogdan mahsulot tanlang va savatga qo'shing
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {items.map((item) => (
            <div
              key={item.productId}
              className="flex gap-4 items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
            >
              {/* Image */}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sticky top-24 space-y-4">
            <h3 className="font-semibold text-slate-800">Buyurtma</h3>
